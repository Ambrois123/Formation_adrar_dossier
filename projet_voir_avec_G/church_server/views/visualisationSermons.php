...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
        ...
        <td>
            <form method="POST" action="<?= URL ?>admin/sermons/validateSuppression" onsubmit="return confirm('Voulez-vous vraiment supprimer ce sermon ?')">
                <input type="hidden" name="sermon_id" value="<?=$sermon['id']?>">
        ...
        <?php else : ?>
            <form method="POST" action="<?= URL ?>admin/sermons/validateModification">
                <tr>
                    <td><?=$sermon['id']?></td>
